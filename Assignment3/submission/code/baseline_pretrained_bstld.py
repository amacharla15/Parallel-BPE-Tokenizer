import os
import json
from ultralytics import YOLO

TEST_TXT = "data/processed/bstld_oneclass/test.txt"
OUT_DIR = "results/baseline_pretrained"
CONF = 0.25
IOU_MATCH = 0.50
IMG_SIZE = 1280
TRAFFIC_LIGHT_CLASS_COCO = 9

os.makedirs(OUT_DIR, exist_ok=True)
os.makedirs(os.path.join(OUT_DIR, "samples"), exist_ok=True)

def read_gt_yolo(label_path):
    boxes = []
    if not os.path.isfile(label_path):
        return boxes
    with open(label_path, "r") as f:
        for line in f:
            parts = line.strip().split()
            if len(parts) != 5:
                continue
            cls_id = int(float(parts[0]))
            cx = float(parts[1])
            cy = float(parts[2])
            w = float(parts[3])
            h = float(parts[4])
            x1 = cx - w / 2.0
            y1 = cy - h / 2.0
            x2 = cx + w / 2.0
            y2 = cy + h / 2.0
            boxes.append([x1, y1, x2, y2, cls_id])
    return boxes

def xyxy_norm(box, img_w, img_h):
    x1, y1, x2, y2 = box
    return [
        x1 / img_w,
        y1 / img_h,
        x2 / img_w,
        y2 / img_h,
    ]

def iou(a, b):
    ax1, ay1, ax2, ay2 = a
    bx1, by1, bx2, by2 = b

    inter_x1 = max(ax1, bx1)
    inter_y1 = max(ay1, by1)
    inter_x2 = min(ax2, bx2)
    inter_y2 = min(ay2, by2)

    inter_w = max(0.0, inter_x2 - inter_x1)
    inter_h = max(0.0, inter_y2 - inter_y1)
    inter = inter_w * inter_h

    area_a = max(0.0, ax2 - ax1) * max(0.0, ay2 - ay1)
    area_b = max(0.0, bx2 - bx1) * max(0.0, by2 - by1)

    union = area_a + area_b - inter
    if union <= 0.0:
        return 0.0
    return inter / union

with open(TEST_TXT, "r") as f:
    image_paths = [line.strip() for line in f if line.strip()]

model = YOLO("yolov8n.pt")

tp = 0
fp = 0
fn = 0

images_with_gt = 0
images_with_pred = 0
images_with_match = 0

saved = 0
sample_limit = 20

for idx, image_path in enumerate(image_paths):
    label_path = os.path.splitext(image_path)[0] + ".txt"
    gt_boxes_all = read_gt_yolo(label_path)
    gt_boxes = [b[:4] for b in gt_boxes_all]

    if len(gt_boxes) > 0:
        images_with_gt += 1

    results = model.predict(
        source=image_path,
        conf=CONF,
        imgsz=IMG_SIZE,
        verbose=False,
        save=False
    )

    r = results[0]
    img_h, img_w = r.orig_shape

    pred_boxes = []
    pred_scores = []

    if r.boxes is not None:
        cls_list = r.boxes.cls.tolist()
        conf_list = r.boxes.conf.tolist()
        xyxy_list = r.boxes.xyxy.tolist()

        for c, s, b in zip(cls_list, conf_list, xyxy_list):
            if int(c) != TRAFFIC_LIGHT_CLASS_COCO:
                continue
            pred_boxes.append(xyxy_norm(b, img_w, img_h))
            pred_scores.append(float(s))

    if len(pred_boxes) > 0:
        images_with_pred += 1

    matched_gt = set()
    matched_pred = set()

    for pi in range(len(pred_boxes)):
        best_iou = 0.0
        best_gi = -1
        for gi in range(len(gt_boxes)):
            if gi in matched_gt:
                continue
            cur_iou = iou(pred_boxes[pi], gt_boxes[gi])
            if cur_iou > best_iou:
                best_iou = cur_iou
                best_gi = gi

        if best_gi != -1 and best_iou >= IOU_MATCH:
            matched_gt.add(best_gi)
            matched_pred.add(pi)

    tp += len(matched_pred)
    fp += len(pred_boxes) - len(matched_pred)
    fn += len(gt_boxes) - len(matched_gt)

    if len(matched_pred) > 0:
        images_with_match += 1

    if saved < sample_limit:
        plotted = r.plot()
        out_path = os.path.join(OUT_DIR, "samples", f"sample_{saved:03d}.jpg")
        import cv2
        cv2.imwrite(out_path, plotted)
        saved += 1

precision = tp / (tp + fp) if (tp + fp) > 0 else 0.0
recall = tp / (tp + fn) if (tp + fn) > 0 else 0.0
f1 = 2 * precision * recall / (precision + recall) if (precision + recall) > 0 else 0.0

metrics = {
    "model": "yolov8n.pt",
    "dataset": "BSTLD one-class test split",
    "confidence_threshold": CONF,
    "iou_match_threshold": IOU_MATCH,
    "imgsz": IMG_SIZE,
    "num_test_images": len(image_paths),
    "images_with_gt": images_with_gt,
    "images_with_pred": images_with_pred,
    "images_with_match": images_with_match,
    "tp": tp,
    "fp": fp,
    "fn": fn,
    "precision": precision,
    "recall": recall,
    "f1": f1,
}

with open(os.path.join(OUT_DIR, "baseline_metrics.json"), "w") as f:
    json.dump(metrics, f, indent=2)

print(json.dumps(metrics, indent=2))
print("saved_samples_dir =", os.path.abspath(os.path.join(OUT_DIR, "samples")))
